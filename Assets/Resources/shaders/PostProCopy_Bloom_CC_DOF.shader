ýKShader "Corona/PostProcess/Copy_Bloom_CC_DOF" {
Properties {
...
void main ()
{
  ...
  lowp float tmpvar_6;
  tmpvar_6 = min ((dot (tmpvar_5, tmpvar_5) * 10.0), 1.0);
  col_2.xyz = (((1.0 - tmpvar_6) * tmpvar_3.xyz) + (tmpvar_6 * tmpvar_4.xyz));
  col_2.x = (texture2D (_RampTex, col_2.xx).x + 1e-05);
  col_2.y = (texture2D (_RampTex, col_2.yy).y + 2e-05);
  col_2.z = (texture2D (_RampTex, col_2.zz).z + 3e-05);
  highp vec3 tmpvar_7;
  tmpvar_7 = (col_2.xyz + (tmpvar_4.w * _Bloomification));
  col_2.xyz = tmpvar_7;
  tmpvar_1 = col_2;
...
highp vec4 frag( in v2f i ) {
    ...
    #line 340
    lowp float scale = (d * 10.0);
    scale = min( scale, 1.0);
    col.xyz = (((1.0 - scale) * col.xyz) + (scale * bloom.xyz));
