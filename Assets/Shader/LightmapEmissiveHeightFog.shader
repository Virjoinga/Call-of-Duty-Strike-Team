...
void main ()
{
  ...
  tmpvar_10 = min (clamp (((tmpvar_7.z * _FogRange.x) + (_FogRange.y + 1.0)), _FogRange.z, 1.0), clamp ((_FogHeight.y * (_FogHeight.x + (_Object2World * _glesVertex).y)), 0.0, 1.0));
...
void main ()
{
  lowp vec4 tmpvar_1;
  tmpvar_1.w = 1.0;
  tmpvar_1.xyz = ((((texture2D (_MainTex, xlv_TEXCOORD0).xyz * texture2D (unity_Lightmap, xlv_TEXCOORD1).xyz) * xlv_TEXCOORD5.w) + xlv_TEXCOORD5.xyz) + texture2D (_EmissiveTex, xlv_TEXCOORD0).xyz);
...
v2f vert( in appdata_full v ) {
    ...
    #line 420
    visibility = min( visibility, xll_saturate_f((_FogHeight.y * (_FogHeight.x + (_Object2World * v.vertex).y))));
...
lowp vec4 frag( in v2f i ) {
    ...
    emissive = vec3( texture( _EmissiveTex, i.uv1));
