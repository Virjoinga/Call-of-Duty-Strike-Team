...
void main ()
{
  ...
  highp vec2 P_3;
  P_3 = (xlv_TEXCOORD6 - floor(xlv_TEXCOORD6));
  diffusemap_1.xyz = clamp (((tmpvar_2.xyz * texture2D (_DetailTex, P_3).xyz) * _DetailPower), 0.0, 1.0);
  lowp vec4 tmpvar_4;
  tmpvar_4.w = 1.0;
  tmpvar_4.xyz = (((diffusemap_1.xyz * texture2D (unity_Lightmap, xlv_TEXCOORD1).xyz) * xlv_TEXCOORD5.w) + xlv_TEXCOORD5.xyz);
...
lowp vec4 frag( in v2f i ) {
    ...
    lowp vec3 detailCol = vec3( texture( _DetailTex, (i.uvDet - floor(i.uvDet))));
    diffusemap.xyz = xll_saturate_vf3(((diffusemap.xyz * detailCol.xyz) * _DetailPower));
    #line 444
