ýGShader "Corona/Effects/BlobShadow" {
Properties {
...
void main ()
{
  ...
  lowp vec4 tmpvar_2;
  tmpvar_2 = mix (vec4(1.0, 1.0, 1.0, 1.0), _Color, texture2D (_MainTex, xlv_TEXCOORD0).wwww);
  tmpvar_1 = tmpvar_2;
...
mediump vec4 frag( in Input IN ) {
    #line 399
    return mix( vec4( 1.0, 1.0, 1.0, 1.0), _Color, vec4( texture( _MainTex, IN.uv_MainTex.xy).w));
}
