Shader "Hidden/Probe/[Normal] [Spec] [SpecLookup] [NoFog]" 
{
    Properties
    {
        _MainTex ("Base (RGB)", 2D) = "white" {}
        _SpecMap ("Specular Mask (RGB)", 2D) = "white" {}
        _BumpMap ("Normalmap", 2D) = "bump" {}
        _SpecLookup ("Specular Power", 2D) = "black" {}
    }
    SubShader
    {
        Tags { "LIGHTMODE"="ForwardBase" "RenderType"="Opaque" }
        Pass
        {
            Tags { "LIGHTMODE"="ForwardBase" "RenderType"="Opaque" }
            Fog { Mode Off }
            CGPROGRAM
            #pragma vertex vert
            #pragma fragment frag
            #include "UnityCG.cginc"
            float4 _FogRange;
            float3 _FogParams;
            float3 _AmbientLight;
            float4 cC;
            float4 cBb;
            float4 cBg;
            float4 cBr;
            float4 cAb;
            float4 cAg;
            float4 cAr;
            float3 _DepthBand;
            float3 _SpecDir;
            sampler2D _SpecMap;
            sampler2D _BumpMap;
            sampler2D _MainTex;
            float _SpecPower;
            struct appdata_t
            {
                float4 tangent : TANGENT;
                float4 texcoord0 : TEXCOORD0;
                float3 normal : NORMAL;
                float4 vertex : POSITION;
            };
            struct v2f
            {
                float4 texcoord5 : TEXCOORD5;
                float3 texcoord3 : TEXCOORD3;
                float3 texcoord2 : TEXCOORD2;
                float4 texcoord1 : TEXCOORD1;
                float2 texcoord0 : TEXCOORD0;
                float4 vertex : POSITION;
            };
            v2f vert(appdata_t v)
            {
                v2f o;
                float4 tmpvar_1;
                tmpvar_1.xyz = normalize(v.tangent.xyz);
                tmpvar_1.w = v.tangent.w;
                float3 tmpvar_2;
                tmpvar_2 = normalize(v.normal);
                float2 tmpvar_3;
                float4 tmpvar_4;
                float3 tmpvar_5;
                float3 tmpvar_6;
                float4 tmpvar_7;
                float4 tmpvar_8;
                tmpvar_8.w = 1.0;
                tmpvar_8.xyz = v.vertex.xyz;
                float4 tmpvar_9;
                tmpvar_9 = UnityObjectToClipPos(tmpvar_8);
                float4 tmpvar_10;
                tmpvar_10.x = tmpvar_9.x;
                tmpvar_10.y = tmpvar_9.y;
                tmpvar_10.z = ((tmpvar_9.z * _DepthBand.z) + (tmpvar_9.w * _DepthBand.y));
                tmpvar_10.w = tmpvar_9.w;
                float2 tmpvar_11;
                tmpvar_11 = v.texcoord0.xy;
                tmpvar_3 = tmpvar_11;
                float3 tmpvar_12;
                float3 dir_13;
                dir_13 = _SpecDir;
                float4 tmpvar_14;
                tmpvar_14.w = 0.0;
                tmpvar_14.xyz = dir_13;
                tmpvar_12 = mul(unity_WorldToObject, tmpvar_14).xyz;
                float4 tmpvar_15;
                tmpvar_15.w = 1.0;
                tmpvar_15.xyz = _WorldSpaceCameraPos;
                float3 tmpvar_16;
                tmpvar_16 = (mul(unity_WorldToObject, tmpvar_15).xyz - v.vertex.xyz);
                float4 tmpvar_17;
                tmpvar_17.w = 0.0;
                tmpvar_17.xyz = tmpvar_2;
                float3 tmpvar_18;
                tmpvar_18 = normalize(mul(unity_ObjectToWorld, tmpvar_17).xyz);
                float3 tmpvar_19;
                tmpvar_19 = (((tmpvar_2.yzx * tmpvar_1.zxy) - (tmpvar_2.zxy * tmpvar_1.yzx)) * v.tangent.w);
                float3 tmpvar_20;
                tmpvar_20.x = dot (tmpvar_16, tmpvar_1.xyz);
                tmpvar_20.y = dot (tmpvar_16, tmpvar_19);
                tmpvar_20.z = dot (tmpvar_16, tmpvar_2);
                float3 tmpvar_21;
                tmpvar_21 = normalize(tmpvar_20);
                tmpvar_5 = tmpvar_21;
                float3 tmpvar_22;
                tmpvar_22.x = dot (tmpvar_12, tmpvar_1.xyz);
                tmpvar_22.y = dot (tmpvar_12, tmpvar_19);
                tmpvar_22.z = dot (tmpvar_12, tmpvar_2);
                tmpvar_6 = tmpvar_22;
                float tmpvar_23;
                tmpvar_23 = max (0.0, dot (tmpvar_18, _SpecDir));
                tmpvar_4.w = tmpvar_23;
                float4 tmpvar_24;
                tmpvar_24.w = 1.0;
                tmpvar_24.xyz = tmpvar_18;
                float3 tmpvar_25;
                float3 x2_26;
                float3 x1_27;
                float4 tmpvar_28;
                tmpvar_28.xyz = float3(0.0, 0.0, 0.0);
                tmpvar_28.w = _AmbientLight.x;
                x1_27.x = dot ((cAr + tmpvar_28), tmpvar_24);
                float4 tmpvar_29;
                tmpvar_29.xyz = float3(0.0, 0.0, 0.0);
                tmpvar_29.w = _AmbientLight.y;
                x1_27.y = dot ((cAg + tmpvar_29), tmpvar_24);
                float4 tmpvar_30;
                tmpvar_30.xyz = float3(0.0, 0.0, 0.0);
                tmpvar_30.w = _AmbientLight.z;
                x1_27.z = dot ((cAb + tmpvar_30), tmpvar_24);
                float4 tmpvar_31;
                tmpvar_31 = (tmpvar_18.xyzz * tmpvar_18.yzzx);
                x2_26.x = dot (cBr, tmpvar_31);
                x2_26.y = dot (cBg, tmpvar_31);
                x2_26.z = dot (cBb, tmpvar_31);
                tmpvar_25 = ((x1_27 + x2_26) + (cC.xyz * ((tmpvar_18.x * tmpvar_18.x) - (tmpvar_18.y * tmpvar_18.y))));
                tmpvar_4.xyz = tmpvar_25;
                float tmpvar_32;
                tmpvar_32 = clamp (((tmpvar_10.z * _FogRange.x) + (_FogRange.y + 1.0)), _FogRange.z, 1.0);
                float4 tmpvar_33;
                tmpvar_33.xyz = (_FogParams - (_FogParams * tmpvar_32));
                tmpvar_33.w = tmpvar_32;
                tmpvar_7 = tmpvar_33;
                o.vertex = tmpvar_10;
                o.texcoord0 = tmpvar_3;
                o.texcoord1 = tmpvar_4;
                o.texcoord2 = tmpvar_5;
                o.texcoord3 = tmpvar_6;
                o.texcoord5 = tmpvar_7;
                return o;
            }
            float4 frag(v2f i) : SV_TARGET
            {
                float tmpvar_1;
                float spec_2;
                float tmpvar_3;
                tmpvar_3 = max (0.0, dot (normalize((normalize(i.texcoord3) + normalize(i.texcoord2))), normalize((tex2D (_BumpMap, i.texcoord0).xyz - 0.5))));
                spec_2 = tmpvar_3;
                float tmpvar_4;
                tmpvar_4 = pow (spec_2, _SpecPower);
                spec_2 = tmpvar_4;
                tmpvar_1 = tmpvar_4;
                float4 tmpvar_5;
                tmpvar_5.w = 1.0;
                tmpvar_5.xyz = ((((tex2D (_MainTex, i.texcoord0).xyz + (tmpvar_1.xxx * tex2D (_SpecMap, i.texcoord0).xyz)) * i.texcoord1.xyz) * i.texcoord5.w) + i.texcoord5.xyz);
                return tmpvar_5;
            }
            ENDCG
        }
    }
    Fallback Off
}