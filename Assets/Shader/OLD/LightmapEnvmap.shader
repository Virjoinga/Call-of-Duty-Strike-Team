    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                float4 tmpvar_1;
                tmpvar_1.w = 1.0;
                tmpvar_1.xyz = ((((tex2D (_MainTex, i.texcoord0).xyz * UNITY_SAMPLE_TEX2D (unity_Lightmap, i.texcoord1).xyz) + ((texCUBE (_ThemedCube, i.texcoord4).xyz * (_Reflectivity * 0.5)) * tex2D (_SpecMap, i.texcoord0).xyz)) * i.texcoord5.w) + i.texcoord5.xyz);
                return tmpvar_1;
            }
            ENDCG
        }
