...
public class EZScreenPlacement : MonoBehaviour, IUseCamera
{
	...
	public Vector3 smallScreenDevicePos = Vector3.forward;
	...
	public void Start()
	{
		...
		if (Application.isPlaying)
		{
			...
			if (TBFUtils.UseAlternativeLayout() && UseSmallScreenPos)
			{
				screenPos.x = smallScreenDevicePos.x;
				screenPos.y = smallScreenDevicePos.y;
			}
			if (flag)
			{
				screenPos.x *= 2f;
	...
	public void PositionOnScreen()
	{
		if (m_awake)
		{
			...
			base.transform.position = SnapWorldPosToNearestPixel(renderCamera, base.transform.position);
			SendMessage("OnReposition", SendMessageOptions.DontRequireReceiver);
		}
	}

	public static Vector3 SnapWorldPosToNearestPixel(Camera cam, Vector3 pos)
	{
