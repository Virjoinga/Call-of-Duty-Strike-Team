...
public class SpriteText : MonoBehaviour, IUseCamera
{
	...
	private static bool mVertOffsetHackSet = false;

	public static float mVertOffsHack;
	...
	public float TotalScreenHeight
	{
		get
		{
			if (renderCamera == null)
			...
			float distanceToPoint = plane.GetDistanceToPoint(base.transform.position);
			worldUnitsPerScreenPixel = Vector3.Distance(renderCamera.ScreenToWorldPoint(new Vector3(0f, 1f, distanceToPoint)), renderCamera.ScreenToWorldPoint(new Vector3(0f, 0f, distanceToPoint)));
			return totalHeight / worldUnitsPerScreenPixel;
		}
	}
	...
	protected virtual void Awake()
	{
		...
		{
			m_awake = true;
			if (!mVertOffsetHackSet)
			{
				mVertOffsHack = ((!TBFUtils.IsRetinaHdDevice()) ? 0.001f : 0.00053f);
				mVertOffsetHackSet = true;
			}
	...
	public virtual void Start()
	{
		...
		UpdateMesh();
		mesh.MarkDynamic();
		base.transform.position = EZScreenPlacement.SnapWorldPosToNearestPixel(renderCamera, base.transform.position);
	}
	...
	protected void ProcessString(string str)
	{
		...
		str = FixUpNewlineCharacters(str);
		int num5;
		if (parseColorTags)
		{
			num5 = 0;
		...
		if (colorTags.Count < 1)
		{
			...
			displayString = FixUpConstantSpaceCharacters(displayString);
	...
	protected string FixUpNewlineCharacters(string str)
	{
	...
	protected string FixUpConstantSpaceCharacters(string str)
	{
		string text = string.Empty;
		string[] separator = new string[2] { "\\c", "\\\nc" };
		string[] array = str.Split(separator, StringSplitOptions.None);
		for (int i = 0; i < array.Length; i++)
		{
			text += array[i];
	...
	protected void Layout_Single_Line()
	{
		...
		float num2 = (totalWidth = ((displayString.Length != 1) ? (spriteFont.GetWidth(displayString) * worldUnitsPerTexel * characterSpacing) : (spriteFont.GetWidth(displayString) * worldUnitsPerTexel)));
		totalHeight = characterSize;
	...
	protected void Layout_Multiline(string[] lines)
	{
		...
		int num2 = 0;
		float num3 = lineSpaceSize - characterSize;
		totalHeight = characterSize * (float)lines.Length + num3 * ((float)lines.Length - 1f);
		...
		case Anchor_Pos.Upper_Left:
			...
			bottomRight = new Vector3(num, 0f - totalHeight, offsetZ);
			break;
		case Anchor_Pos.Upper_Center:
			vector = (topLeft = new Vector3(num * -0.5f, 0f, offsetZ));
			bottomRight = new Vector3(num * 0.5f, 0f - totalHeight, offsetZ);
			break;
		case Anchor_Pos.Upper_Right:
			vector = (topLeft = new Vector3(0f - num, 0f, offsetZ));
			bottomRight = new Vector3(0f, 0f - totalHeight, offsetZ);
			break;
		case Anchor_Pos.Middle_Left:
			vector = (topLeft = new Vector3(0f, totalHeight * 0.5f, offsetZ));
			bottomRight = new Vector3(num, totalHeight * -0.5f, offsetZ);
			break;
		case Anchor_Pos.Middle_Center:
			vector = (topLeft = new Vector3(num * -0.5f, totalHeight * 0.5f, offsetZ));
			bottomRight = new Vector3(num * 0.5f, totalHeight * -0.5f, offsetZ);
			break;
		case Anchor_Pos.Middle_Right:
			vector = (topLeft = new Vector3(0f - num, totalHeight * 0.5f, offsetZ));
			bottomRight = new Vector3(0f, totalHeight * -0.5f, offsetZ);
			break;
		case Anchor_Pos.Lower_Left:
			vector = (topLeft = new Vector3(0f, totalHeight, offsetZ));
			bottomRight = new Vector3(num, 0f, offsetZ);
			break;
		case Anchor_Pos.Lower_Center:
			vector = (topLeft = new Vector3(num * -0.5f, totalHeight, offsetZ));
			bottomRight = new Vector3(num * 0.5f, 0f, offsetZ);
			break;
		case Anchor_Pos.Lower_Right:
			vector = (topLeft = new Vector3(0f - num, totalHeight, offsetZ));
	...
	protected void BuildCharacter(int vertNum, int charNum, Vector3 upperLeft, ref SpriteChar ch)
	{
		...
		float num = mVertOffsHack;
		for (int i = 0; i < 4; i++)
		{
			vertices[vertNum + i].x += num;
			vertices[vertNum + i].y += num;
		}
	}
	...
	protected void Layout_Line(Vector3 startPos, string txt, int charIdx)
	{
		...
		if (txt.Length > 1)
		{
			num = (GetWidth(txt) - spriteFont.GetWidth(txt) * worldUnitsPerTexel) / ((float)txt.Length - (1f + characterSpacing * 0.5f));
		}
		if (num == 0f)
		{
			num2 = characterSpacing;
		}
		...
		for (int i = 1; i < txt.Length; i++)
		{
			if (ch != null)
			{
				startPos.x += ch.xAdvance * worldUnitsPerTexel * num2 + num;
			}
			ch = spriteFont.GetSpriteChar(txt[i]);
			if (ch != null)
			{
				startPos.x += ch.GetKerning(txt[i - 1]) * worldUnitsPerTexel * num2;
				Vector3 upperLeft = startPos + new Vector3(ch.xOffset * worldUnitsPerTexel, ch.yOffset * worldUnitsPerTexel, 0f);
