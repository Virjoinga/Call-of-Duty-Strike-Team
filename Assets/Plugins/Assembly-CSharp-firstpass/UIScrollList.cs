...
public class UIScrollList : MonoBehaviour, IEZDragDrop, IUIObject
{
	...
	public MonoBehaviour scriptWithMethodToInvokeForScrollSfx;

	public string methodToInvokeForScrollSfx;
	...
	private float mLastScrollSfxPos;
	...
	public bool Contains(IUIListObject item)
	{
	...
	private void DoScrollingSfx()
	{
		float num = scrollPos * contentExtents;
		if (isScrolling && viewableAreaActual.y < contentExtents)
		{
			float num2 = 5f;
			bool flag = false;
			if (Mathf.Abs(mLastScrollSfxPos - num) > num2)
			{
				mLastScrollSfxPos = num;
				flag = true;
			}
			if (flag && scriptWithMethodToInvokeForScrollSfx != null && methodToInvokeForScrollSfx != null)
			{
				scriptWithMethodToInvokeForScrollSfx.Invoke(methodToInvokeForScrollSfx, 0f);
			}
		}
		else
		{
