...
{
	public bool m_verticalProgress;
	...
	{
		if (m_verticalProgress)
		{
			TruncateTop(m_value);
			if (emptySprite != null)
			{
				emptySprite.TruncateBottom(1f - m_value);
			}
			for (int i = 0; i < filledLayers.Length; i++)
			{
				filledLayers[i].TruncateTop(m_value);
			}
			for (int j = 0; j < emptyLayers.Length; j++)
			{
				emptyLayers[j].TruncateBottom(1f - m_value);
			}
		}
		else
		{
			...
			for (int k = 0; k < filledLayers.Length; k++)
			{
				filledLayers[k].TruncateRight(m_value);
			}
			for (int l = 0; l < emptyLayers.Length; l++)
			{
				emptyLayers[l].TruncateLeft(1f - m_value);
			}
		}
	}
	...
	public void SetEmptySpriteColor(Color c)
	{
		if (emptySprite != null)
		{
			emptySprite.SetColor(c);
