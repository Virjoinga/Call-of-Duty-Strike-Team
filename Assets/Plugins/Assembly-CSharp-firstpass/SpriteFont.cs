...
public class SpriteFont
{
	...
	public float CharacterSpacing
	{
		...
		set
		{
			...
			for (int i = 0; i < chars.Length; i++)
			{
				...
				if (chars[i].kernings == null || chars[i].origKernings == null)
				{
					continue;
				}
				...
				{
					if (chars[i].origKernings.ContainsKey(array[j]))
					{
						chars[i].kernings[array[j]] = (float)chars[i].origKernings[array[j]] * charSpacing;
					}
				}
			}
		}
	...
	private bool KerningParser(string line)
	{
		...
		{
			spriteChar.kernings = new Hashtable();
			spriteChar.origKernings = new Hashtable();
		}
