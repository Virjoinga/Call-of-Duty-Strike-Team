...
public class UIManager : MonoBehaviour
{
	...
	public delegate void ShowDebugPressDelegate(Vector3 hudPos);
	...
	public ShowDebugPressDelegate ShowDebugPress;
		...
		set
		{
			...
			if (focusObj != null)
			{
				...
				if (!Application.isEditor)
				{
					...
					iKeyboard = TouchScreenKeyboard.Open(controlText, kbInfo.type, kbInfo.autoCorrect, kbInfo.multiline, kbInfo.secure, kbInfo.alert, empty);
	...
	public bool DidPointerHitUI(int id)
	{
		...
		id = Mathf.Clamp(id, 0, usedPointers.Length - 1);
		...
		else
		{
			...
			case POINTER_INFO.INPUT_EVENT.DRAG:
				if (curPtr.evt == POINTER_INFO.INPUT_EVENT.RELEASE || curPtr.evt == POINTER_INFO.INPUT_EVENT.TAP)
				{
					...
					{
						if (curPtr.targetGO == hit.collider.gameObject && curPtr.targetObj != null)
						{
							tempObj = curPtr.targetObj;
						}
						else
						{
					...
					if (tempObj != curPtr.targetObj)
					{
						...
						if (!blockInput)
						{
							...
							curPtr.targetGO = ((curPtr.targetObj == null) ? null : curPtr.targetObj.gameObject);
						}
			...
			case POINTER_INFO.INPUT_EVENT.PRESS:
				if (Physics.Raycast(curPtr.ray, out hit, curPtr.rayDepth, curPtr.layerMask))
				{
					...
					if (ShowDebugPress != null)
					{
						ShowDebugPress(hit.point);
					}
					curPtr.hitInfo = hit;
					if (tempObj != null)
					{
	...
	protected void PollMouse(ref POINTER_INFO curPtr)
	{
		...
		else if (down && !curPtr.active)
		{
			...
			curPtr.isTap = true;
			curPtr.activeTime = Time.realtimeSinceStartup;
			curPtr.targetObj = null;
			curPtr.targetGO = null;
		}
		else if (!down && curPtr.active)
	...
	protected void PollTouchpad()
	{
		...
		for (int i = 0; i < numActivePointers; i++)
		{
			...
			case TouchPhase.Moved:
				...
				pointers[0, num].inputDelta = new Vector3(touch.position.x - pointers[0, num].devicePos.x, touch.position.y - pointers[0, num].devicePos.y, 0f);
			...
			case TouchPhase.Began:
				...
				pointers[0, num].activeTime = Time.realtimeSinceStartup;
				pointers[0, num].targetObj = null;
				break;
			case TouchPhase.Ended:
			case TouchPhase.Canceled:
				if (Time.realtimeSinceStartup - pointers[0, num].activeTime > mTapTime)
				{
					pointers[0, num].isTap = false;
				}
				if (pointers[0, num].isTap)
	...
	protected void PollRay()
	{
		...
		else if (rayActive && !rayPtr.active)
		{
			...
			rayPtr.isTap = true;
			rayPtr.activeTime = Time.realtimeSinceStartup;
	...
	public void Detarget(int pointerID)
	{
		...
		for (int i = 0; i < uiCameras.Length; i++)
		{
			if (uiCameras[i].camera != null && uiCameras[i].camera.gameObject.activeInHierarchy && pointers[i, pointerID].targetObj != null)
			{
				...
				pointers[i, pointerID].targetGO = null;
			}
		}
	}
		...
		for (int i = 0; i < numActivePointers; i++)
		{
			...
			for (int j = 0; j < uiCameras.Length; j++)
			{
				if (uiCameras[j].camera != null && uiCameras[j].camera.gameObject.activeInHierarchy && pointers[j, activePointers[i]].targetObj != null)
				{
					...
					pointers[j, activePointers[i]].targetGO = null;
				}
			}
		}
	}
	...
	public void Retarget(IUIObject oldObj, IUIObject newObj)
	{
		...
		for (int i = 0; i < numActivePointers; i++)
		{
			for (int j = 0; j < uiCameras.Length; j++)
			{
				if (uiCameras[j].camera != null && uiCameras[j].camera.gameObject.activeInHierarchy && pointers[j, activePointers[i]].targetObj != null && pointers[j, activePointers[i]].targetObj == oldObj)
				{
					...
					pointers[j, activePointers[i]].targetGO = ((newObj == null) ? null : newObj.gameObject);
				}
			}
		}
		if (rayPtr.targetObj == oldObj)
		{
			...
			rayPtr.targetGO = ((newObj == null) ? null : newObj.gameObject);
		}
	}
