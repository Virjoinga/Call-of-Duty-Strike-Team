...
internal class Weapon_Vektor : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_Vektor mDescriptor;
	...
	public Weapon_Vektor(WeaponDescriptor_Vektor descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.Type25MuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.Type25CasingEject);
		Reset();
	}
	...
	{
		return WeaponDescriptor.WeaponClass.SubMachineGun;
	}
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			WeaponSFX.Instance.VektorFire.Play(owner.gameObject);
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.VektorMagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.VektorMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.VektorChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.VektorChargeForward.Play(gameObject);
					break;
				}
			}
		}
