...
public class Weapon_SVUAS : IWeapon, IWeaponADS, IWeaponAI, IWeaponControl, IWeaponSway, IWeaponStats
{
	private WeaponDescriptor_SVUAS mDescriptor;
	...
	public Weapon_SVUAS(WeaponDescriptor_SVUAS descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.SVUASMuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.SVUASCasingEject);
		Reset();
	}
	...
	{
		if (mWeaponEquip.IsPuttingAway() || mWeaponEquip.IsTakingOut() || IsReloading() || mHasDepressed || mBulletGapCooldown > 0f)
		{
			return false;
		}
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.SVUASFire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.SVUASFire.Play(owner.gameObject, m_AudioFilter);
			}
		}
		...
		if (owner.IsFirstPerson)
		{
			Vector3 origin = ViewModelRig.Instance().Fire(GetId(), ViewModelRig.FireEffects.MuzzleFlash | ViewModelRig.FireEffects.CasingEject);
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.SVUASMagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.SVUASMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.SVUASChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.SVUASChargeForward.Play(gameObject);
					break;
				}
			}
		}
