...
public class NavMeshUtils
{
	...
	public static NavMeshHit SampleNavMesh(Vector3 position)
	{
		return SampleNavMesh(position, -1);
	}

	public static NavMeshHit SampleNavMesh(Vector3 position, int layerMask)
	{
		float num = 0.1f;
		NavMeshHit hit = default(NavMeshHit);
		while (!hit.hit && num < 128f)
		{
			NavMesh.SamplePosition(position, out hit, num, layerMask);
			num *= 2f;
		}
		return hit;
	}
	...
	public static string GetNavMeshLayerName(int mask)
	{
		...
		{
			int navMeshLayerFromName = NavMesh.GetNavMeshLayerFromName(mPossibleLayer);
	...
	public static string FindClosestPortal(Vector3 testPosition, float maxDistance)
	{
		...
		for (int i = 0; i < 32; i++)
		{
			...
			int navMeshLayerFromName = NavMesh.GetNavMeshLayerFromName(text);
			NavMeshHit hit;
			if (navMeshLayerFromName != -1 && NavMesh.SamplePosition(testPosition, out hit, maxDistance, 1 << navMeshLayerFromName))
			{
