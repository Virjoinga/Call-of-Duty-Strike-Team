...
public class ViewModelBlendTree_SVUAS : IViewModelWeaponBlendTree
{
	...
	public ViewModelBlendTree_SVUAS(GameObject weapon, WeaponCoreAnims coreAnims, SVUASAnims specificAnims)
	{
	...
	public void Update(IWeapon weaponState)
	{
		...
		if (weapon_SVUAS.IsReloading())
		{
			AnimationState animationState = ((!weapon_SVUAS.IsTacticalReload()) ? mReloadEmpty : mReloadTactical);
			animationState.enabled = true;
			animationState.layer = 2;
			animationState.weight = 1f;
			animationState.normalizedTime = weapon_SVUAS.GetReloadingAmount();
			mOldTimeInReloadAnim = weapon_SVUAS.PlayReloadSfxFP(mOldTimeInReloadAnim, animationState.normalizedTime * animationState.length, mReloadEventTimes, ref mWhichReloadEvent);
		}
		else if (weapon_SVUAS.IsRecoiling())
		{
			float recoilingAmount = weapon_SVUAS.GetRecoilingAmount();
			float hipsToSightsBlendAmount = weapon_SVUAS.GetHipsToSightsBlendAmount();
