...
public class UnitsInVolumeOverride : ContainerOverride
{
	public MissionObjectiveData m_OverrideData = new MissionObjectiveData();

	public UnitsInVolData m_OverrideVolData = new UnitsInVolData();
	...
	public override void ApplyOverride(Container cont)
	{
		...
		UnitsInVolumeObjective unitsInVolumeObjective = cont.FindComponentOfType(typeof(UnitsInVolumeObjective)) as UnitsInVolumeObjective;
		if (unitsInVolumeObjective != null)
		{
			unitsInVolumeObjective.m_Interface = m_OverrideData;
			unitsInVolumeObjective.m_UnitsVolInterface = m_OverrideVolData;
			GameObject innerObject = cont.GetInnerObject();
			if (innerObject != null && m_OverrideVolData.TriggerVolume == null && unitsInVolumeObjective.m_UnitsVolInterface != null)
			{
				unitsInVolumeObjective.m_UnitsVolInterface.TriggerVolume = innerObject.GetComponent<Collider>();
			}
			m_OverrideData.CopyContainerObjectives(unitsInVolumeObjective);
			m_OverrideVolData.CopyContainerData(unitsInVolumeObjective);
		}
	}
	...
	public override void HookUpVisibleConnections(Container cont)
	{
		...
		if (m_OverrideVolData.Actors == null || m_OverrideVolData.Actors.Count == 0)
		{
			return;
		}
		foreach (GameObject actor in m_OverrideVolData.Actors)
		{
			if (actor != null)
			{
