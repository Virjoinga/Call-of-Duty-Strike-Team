...
public class Weapon_M8A1 : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_M8A1 mDescriptor;
	...
	public Weapon_M8A1(WeaponDescriptor_M8A1 descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.M8A1MuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.M8A1CasingEject);
		Reset();
	}
	...
	{
		return WeaponDescriptor.WeaponClass.AssaultRifle;
	}
	...
	public void Update(float deltaTime, BaseCharacter owner, SoldierFiringState sfs)
	{
		...
		if (mWantsReload && sfs.animsAllowReload && mBurstShotsFired >= 4)
		{
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		mBulletGapCooldown = mDescriptor.TimeBetweenBurst;
		if (owner != null)
		{
			if (owner.IsFirstPerson || m_AudioFilter == null)
			{
				WeaponSFX.Instance.XM8Fire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.XM8Fire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	private bool FireRemainingBurstShots(BaseCharacter owner)
	{
		...
		if (mBurstShotsFired == 3 && mTimeSinceStartOfBurst > num3)
		{
			FireBurstShot(owner, mBurstShotsFired);
			mBurstShotsFired++;
		}
		return mBurstShotsFired == 1 || mBurstShotsFired == 2 || mBurstShotsFired == 3;
	}
	...
	private void FireBurstShot(BaseCharacter owner, int burstCount)
	{
		if (owner != null)
		{
			...
			Vector3 target = crosshairCentre + WeaponUtils.GetFirstPersonShotDirection(owner.myActor, (!owner.IsFirstPerson) ? GetThirdPersonAccuracy() : GetFirstPersonAccuracy());
	...
	{
		float num = mBulletGapCooldown;
		return 1f - Mathf.Clamp01(num / mDescriptor.TimeBetweenBurst);
	}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.XM8MagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.XM8MagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.XM8ChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.XM8ChargeForward.Play(gameObject);
					break;
				}
			}
		}
