...
public class ComboOverride : ContainerOverride
{
	public MissionObjectiveData m_OverrideData = new MissionObjectiveData();

	public ComboObjectiveData m_ComboOverrideData = new ComboObjectiveData();
	...
	public override void ApplyOverride(Container cont)
	{
		...
		ComboObjective comboObjective = cont.FindComponentOfType(typeof(ComboObjective)) as ComboObjective;
		if (comboObjective != null)
		{
			comboObjective.m_Interface = m_OverrideData;
			comboObjective.m_comboInterface = m_ComboOverrideData;
			m_OverrideData.CopyContainerObjectives(comboObjective);
			m_ComboOverrideData.CopyContainerData(comboObjective);
		}
	}
	...
	public override void HookUpVisibleConnections(Container cont)
	{
		...
		if (m_ComboOverrideData.SubObjectives == null || m_ComboOverrideData.SubObjectives.Count == 0)
		{
			return;
		}
		foreach (Container subObjective in m_ComboOverrideData.SubObjectives)
		{
			GameObject gameObject4 = subObjective.gameObject;
			LineFlag inFlag5 = LineFlag.Out;
			Transform inTrans5 = gameObject4.transform;
