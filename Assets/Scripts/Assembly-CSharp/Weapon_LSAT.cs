...
internal class Weapon_LSAT : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_LSAT mDescriptor;
	...
	public Weapon_LSAT(WeaponDescriptor_LSAT descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.LSATMuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.LSATCasingEject);
		Reset();
	}
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.LSATFire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.LSATFire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.LSATChargeBack.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.LSATMagOut.Play(gameObject);
					break;
				case 2:
					WeaponSFX.Instance.LSATMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 3:
					...
					break;
				case 4:
					WeaponSFX.Instance.LSATChargeForward.Play(gameObject);
					break;
				}
			}
		}
