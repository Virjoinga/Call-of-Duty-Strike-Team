...
internal class Weapon_Skorpion : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_Skorpion mDescriptor;
	...
	public Weapon_Skorpion(WeaponDescriptor_Skorpion descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.AN94CasingEject);
		Reset();
	}
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.SkorpionFire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.SkorpionFire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.SkorpionMagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.SkorpionMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.SkorpionChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.SkorpionChargeForward.Play(gameObject);
					break;
				}
			}
		}
