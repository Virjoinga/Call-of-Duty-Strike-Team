...
public class Weapon_QBBLSW : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_QBBLSW mDescriptor;
	...
	public Weapon_QBBLSW(WeaponDescriptor_QBBLSW descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.QBBLSWMuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.QBBLSWCasingEject);
		Reset();
	}
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.QBBFire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.QBBFire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.QBBMagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.QBBMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.QBBChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.QBBChargeForward.Play(gameObject);
					break;
				}
			}
		}
