...
internal class Weapon_XPR50 : IWeapon, IWeaponADS, IWeaponAI, IWeaponControl, IWeaponSway, IWeaponStats
{
	private WeaponDescriptor_XPR50 mDescriptor;
	...
	public Weapon_XPR50(WeaponDescriptor_XPR50 descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
		...
		WeaponUtils.CreateMuzzleFlash(mMuzzleLocator, EffectsController.Instance.Effects.AS50MuzzleFlash);
		mCasingEffect = WeaponUtils.CreateCasingEffect(mWeaponModel.transform.FindInHierarchy("cartridge"), EffectsController.Instance.Effects.AS50CasingEject);
		Reset();
	}
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.AS50Fire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.AS50Fire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.AS50MagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.AS50MagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.AS50ChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.AS50ChargeForward.Play(gameObject);
					break;
				}
			}
		}
