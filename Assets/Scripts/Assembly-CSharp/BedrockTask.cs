...
public class BedrockTask : IDisposable
{
	...
	{
		Bedrock.brTaskStatus status;
		if (Bedrock.getTaskStatus(Handle, out status))
		{
			Status = status;
			return;
		}
		Status = Bedrock.brTaskStatus.BR_TASK_NOT_IN_USE;
		Debug.LogError("Unable to retrieve task status for handle '" + Handle + "'... Assuming BR_TASK_NOT_IN_USE.");
	}
