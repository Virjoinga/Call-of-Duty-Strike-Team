...
internal class Weapon_PDW : IWeapon, IWeaponADS, IWeaponAI, IWeaponStats
{
	private WeaponDescriptor_PDW mDescriptor;
	...
	public Weapon_PDW(WeaponDescriptor_PDW descriptor, GameObject model, float adsModifier, float ammoModifier, float reloadModifier)
	{
	...
	private bool TryToFire(BaseCharacter owner, Vector3 position, Vector3 target)
	{
		...
		if (owner != null)
		{
			...
			{
				WeaponSFX.Instance.PDWFire.Play(owner.gameObject);
			}
			else
			{
				WeaponSFX.Instance.PDWFire.Play(owner.gameObject, m_AudioFilter);
			}
		}
	...
	public float PlayReloadSfxFP(float oldTimeInAnim, float newTimeInAnim, float[,] eventTimes, ref int whichEvent)
	{
		...
		if (eventToPlay != -1 && GameController.Instance.mFirstPersonActor != null)
		{
			...
			if (IsTacticalReload())
			{
				...
				{
				case 0:
					WeaponSFX.Instance.PDWMagOut.Play(gameObject);
					break;
				case 1:
					WeaponSFX.Instance.PDWMagIn.Play(gameObject);
					break;
				}
			}
			else
			{
				...
				case 1:
					...
					break;
				case 2:
					WeaponSFX.Instance.PDWChargeBack.Play(gameObject);
					break;
				case 3:
					WeaponSFX.Instance.PDWChargeForward.Play(gameObject);
					break;
				}
			}
		}
