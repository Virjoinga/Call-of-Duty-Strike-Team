...
public class UnityDebugLogger : ILogger
{
	...
	{
		if (Debug.isDebugBuild && LogLevelExtensions.ShouldLogMessage(LogLevel.Debug, OutputLevel))
		{
	...
	{
		if (Debug.isDebugBuild && LogLevelExtensions.ShouldLogMessage(LogLevel.Log, OutputLevel))
		{
			Debug.Log("[" + _identifier + "]: " + string.Format(formatString, args));
		}
	}
	...
	{
		if (Debug.isDebugBuild && LogLevelExtensions.ShouldLogMessage(LogLevel.Warning, OutputLevel))
		{
	...
	{
		if (Debug.isDebugBuild && LogLevelExtensions.ShouldLogMessage(LogLevel.Error, OutputLevel))
		{
